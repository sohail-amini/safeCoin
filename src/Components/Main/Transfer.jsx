...
export const Transfer = () => {
    const { pendingTransfer, prices, balance, setBalance, setToast } = useContext(GlobalContext)
    ...
    const createTransfer = async (e) => {
        e.preventDefault()
        setLoader(true)
        await axios.post(`${AppSettings.APIserver}/transfer`, { ...transferInfo, senderId: info.id }).then(res => {
    ...
    return (
        <div className="w-full flex-1">
            ...
            <div className="flex  flex-col space-y-4">
                <form className={`w-3/5 mt-4 flex-1 border border-gray-100 p-6 rounded shadow-md ${pendingTransfer && 'opacity-40'}`} onSubmit={createTransfer} >

                    <div class="p-4 mb-4 text-green-800 rounded-lg bg-green-100 dark:bg-gray-800 dark:text-green-400" role="alert">
                        <span class="font-base"> Balance:</span> ฿ {balance} <b className="ml-2">${(balance * prices.btc).toFixed(2)} </b>
                    </div>
                    <div class="mb-6">
                ...
                <div>
                    <h2 className="text-2xl border rounded p-2 mb-2">Recent Transfer</h2>
                    <div class="flex-1  relative shadow-md sm:rounded-lg">
